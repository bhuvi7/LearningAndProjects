...
@Component({
    selector: "fin-02-create-list",
    templateUrl: "./fin-02-create-list.component.html"
})
export class Fin02CreateListComponent implements OnInit {
    ...
    selectedIds(selectedIds) {
        if (this.route.params['_value']['_indicator'] != "undefined") {
            if (this.route.params['_value']['_indicator'] == "new") {
                this.loading = true;
                this.fin02Service.fetchDataForFIN02CreateList(selectedIds).subscribe(fin => {
                ...
                this.fin02Service.fetchDataForFIN02CreateListOlder(selectedIds).subscribe(fin => {
                    if (fin.length == 0) {
                        Swal.fire('', 'All the clinics under this state/district is Approved,check Approved process', 'info');
                        this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
                            dtInstance.destroy();
    ...
    radioSelect() {
        this.router.navigateByUrl('transaction/fin-02-invoice/fin-02-create/' + this.createId);
    }

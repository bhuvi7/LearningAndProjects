...
@Component({
    selector: 'fin-08b-inprogress',
    templateUrl: './fin-08b-inprogress.component.html',
})

export class Fin08bInProgressListComponent implements OnInit {
    ...
    selectedIds(selectedIds) {
        if (this.route.params['_value']['_indicator'] != "undefined") {
            if (this.route.params['_value']['_indicator'] == "new") {
                this.loading = true;
                this.fin08Service.fetchDataForFin08bInProgress('APPROVED BY MOH', selectedIds).subscribe(fin => {
                ...
                this.fin08Service.fetchDataForFin08bInProgressOlder('APPROVED BY MOH', selectedIds).subscribe(fin => {
                    if (fin.length == 0) {
                        Swal.fire('', 'All the clinics under this state/district is Approved,check Approved process', 'info');
                        this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
                            dtInstance.destroy();

...
@Component({
  selector: 'fin-08c-approved-list',
  templateUrl: './fin-08c-approved-list.component.html'
})

export class Fin08cApprovedComponent implements OnInit {
  ...
  selectedIds(selectedIds) {
    ...
    if (this.route.params['_value']['_indicator'] != "undefined") {
      if (this.route.params['_value']['_indicator'] == "new") {
        this.loading=true;
        this.fin08Service.fetchDataForFin08cApproved('APPROVED BY MOH', selectedIds).subscribe(fin => {
        ...
        this.fin08Service.fetchDataForFin08cApprovedOlder('APPROVED BY MOH', selectedIds).subscribe(fin => {
          if (fin.length == 0) {
            Swal.fire('', 'All the clinics under this state/district is Approved,check Approved process', 'info');
          } else {
            fin.forEach((element, i) => {
  ...
  navigateToApproval(id) {
    this.router.navigateByUrl('transaction/fin-02-invoice/fin-08/fin-08c-approve/' + id);
  }
}
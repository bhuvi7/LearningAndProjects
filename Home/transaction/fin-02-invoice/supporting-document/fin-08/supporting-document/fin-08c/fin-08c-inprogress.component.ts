...
@Component({
    selector: 'fin-08c-inprogress',
    templateUrl: './fin-08c-inprogress.component.html',
})

export class Fin08cInProgressComponent implements OnInit {
    ...
    selectedIds(selectedIds) {
        ...
        if (this.route.params['_value']['_indicator'] != "undefined") {
            if (this.route.params['_value']['_indicator'] == "new") {
                this.loading = true;
                this.fin08Service.fetchDataForFin08cInProgress('APPROVED BY MOH', selectedIds).subscribe(fin => {
                ...
                this.fin08Service.fetchDataForFin08cInProgressOlder('APPROVED BY MOH', selectedIds).subscribe(fin => {
                    if (fin.length == 0) {
                        Swal.fire('', 'All the clinics under this state/district is Approved,check Approved process', 'info');
                    } else {
                        fin.forEach((element, i) => {

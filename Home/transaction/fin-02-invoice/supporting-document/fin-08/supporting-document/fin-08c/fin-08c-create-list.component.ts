...
@Component({
    selector: "fin-08c-create-list",
    templateUrl: "./fin-08c-create-list.component.html"
})
export class Fin08cCreateListComponent implements OnInit {
    ...
    selectedIds(selectedIds) {
        ...
        if (this.route.params['_value']['_indicator'] != "undefined") {
            if (this.route.params['_value']['_indicator'] == "new") {
                ...
                this.fin08Service.fetchDataForFin08cCreateList(selectedIds).subscribe(fin => {
                ...
                this.fin08Service.fetchDataForFin08cCreateListOlder(selectedIds).subscribe(fin => {
                    if (fin.length == 0) {
                        Swal.fire('', 'All the clinics under this state/district is Approved,check Approved process', 'info');
                    } else {
                        fin.forEach((element, i) => {
    ...
    radioSelect() {
        ...
        this.router.navigateByUrl('transaction/fin-02-invoice/fin-08/fin-08c-create/' + this.clinicId + '/' + this.month + '/' + this.year);
    }

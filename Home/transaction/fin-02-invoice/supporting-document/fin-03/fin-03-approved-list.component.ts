...
@Component({
  selector: 'fin-03-approved-list',
  templateUrl: 'fin-03-approved-list.component.html'
})

export class Fin03ApprovedListComponent implements OnInit {
  ...
  selectedIds(selectedIds) {
    if (this.route.params['_value']['_indicator'] != "undefined") {
      if (this.route.params['_value']['_indicator'] == "new") {
        this.loading = true;
        this.fin02Service.fetchDataForFin03Approved('APPROVED BY MOH', selectedIds).subscribe(fin => {
        ...
        this.fin02Service.fetchDataForFin03ApprovedOlder('APPROVED BY MOH', selectedIds).subscribe(fin => {
          if (fin.length == 0) {
            Swal.fire('', 'All the clinics under this state/district is Approved,check Approved process', 'info');
            this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
              dtInstance.destroy();

...
@Component({
    selector: 'fin-03-inprogress-list',
    templateUrl: './fin-03-inprogress-list.component.html'
})

export class Fin03InProgressListComponent implements OnInit {
     ...
     selectedIds(selectedIds) {
        if (this.route.params['_value']['_indicator'] != "undefined") {
            if (this.route.params['_value']['_indicator'] == "new") {
                this.loading = true;
                this.fin02Service.fetchDataForFin03InProgress('APPROVED BY MOH', selectedIds).subscribe(fin => {
                ...
                this.fin02Service.fetchDataForFin03InProgressOlder('APPROVED BY MOH', selectedIds).subscribe(fin => {
                    if (fin.length == 0) {
                        Swal.fire('', 'All the clinics under this state/district is Approved,check Approved process', 'info');
                        this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
                            dtInstance.destroy();
    ...
    navigateToApproval(data) {
        this.router.navigateByUrl('transaction/fin-02-invoice/fin-03-approve/' + data);
    }
}
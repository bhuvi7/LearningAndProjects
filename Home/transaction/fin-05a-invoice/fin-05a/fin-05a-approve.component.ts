...
@Component({
    selector: 'fin-05a-approve',
    templateUrl: 'fin-05a-approve.component.html',
    styleUrls: ['fin-05a-approve.component.scss'],
    providers: [NgbModalConfig, NgbModal]
})

export class Fin05aApproveComponent implements OnInit {
    ...
    ngOnInit() {
        if (this.route.params['_value']['_id'] != "undefined") {
            this.route.params.switchMap((par: Params) => this.fin05aService.fetchDataForFin05aApprove(par['_id'])).subscribe(x => {
                this.fin05a = x;
                this.districtId = x.districtId
                this.clinicDataDummy = x.fin05aSiteConformity;
                this.clinicDatas = [{}];
                ...
                this.siteConformityExpenses = this.clinicDatas[0].siteConformityExpenses;
                ...
                this.docRef = this.fin05a.status == 'CREATED' || this.fin05a.status == 'SAVED' ? "" : this.fin05a.invoiceNo;
                this.isApproved = this.fin05a.status == 'APPROVED BY MOH' ? true : false;
                this.showButtons = this.fin05a.status == 'APPROVED BY MOH' ? false : true;
                this.showSaveSubmitButton = this.fin05a.status == 'IN INTERNAL APPROVAL' || this.fin05a.status == 'FOR APPROVAL TO MOH' ? false : true;
    ...
    saveStatus() {
        // delete this.fin05a.fin05aSiteConformity[0];
        this.fin05a.status = "SAVED";
        this.fin05aService.updateFin05aStatus(this.fin05a).subscribe(x => {
            Swal.fire('', 'FIN 05A Invoice saved successfully!!!', 'success');
        })
    }

    submitStatus() {
        // delete this.fin05a.fin05aSiteConformity[0];
        this.fin05a.status = 'IN INTERNAL APPROVAL'
        this.fin05aService.updateFin05aStatus(this.fin05a).subscribe(x => {
            Swal.fire('', 'FIN 05A Invoice submitted successfully!!!', 'success');
            history.back()
    ...
    updateStatus() {
        if (this.fin05a.status == 'IN INTERNAL APPROVAL') { this.fin05a.status = 'FOR APPROVAL TO MOH' }
        else if (this.fin05a.status == 'FOR APPROVAL TO MOH') { this.fin05a.status = 'APPROVED BY MOH' }
        // delete this.fin05a.fin05aSiteConformity[0];
        this.fin05aService.updateFin05aStatus(this.fin05a).subscribe(x => {
            Swal.fire('', 'FIN 05A Invoice approved successfully!!!', 'success');
            history.back()
    ...
    generatePDF() {
        ...
        if (this.fin05a.status == 'FOR APPROVAL TO MOH') { 
            if (this.invoiceDate != '') {
            ...
            content: [
                ...
                },
                {
                    table: {
                        ...
                        body: [
                            [{
                                text: 'INVOICE FOR SITE CONFORMITY',
                                bold: true
                            },],
                        ]
                    },
                ...
                },
                {
                    columns: [
                        ...
                        {},
                        {
                            width: '30%',
                            stack: [
                                { text: 'FORM FIN 05-A', bold: true,alignment:'right' },
                                {
                                    table: {
                                        widths: ['*', 'auto'],
                                        body: [
                ...
                },
                {
                    table: {
                        ...
                        widths: ['*', 'auto', 'auto'],
                        body: [
                            [{ text: 'TYPE OF EXPENSES', bold: true, alignment: 'center' }, { text: 'REFERENCE', bold: true, alignment: 'center' }, { text: 'SITE CONFIRMITY EXPENSES (RM)', bold: true, alignment: 'center' }],
                            ...this.clinicDatas.map(data => {
                                return [data.typeOfExpenses, { text: data.reference, alignment: 'center' }, { text: formatNumber(data.siteConformityExpenses, 'en', '1.2-2'), alignment: 'right' }]
                            }),
                            ['\n', '\n', '\n'],

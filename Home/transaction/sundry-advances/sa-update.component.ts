...
@Component({
  selector: 'sa-update',
  templateUrl: './sa-update.component.html',
  styleUrls: ['./sa-update.component.scss']
})
export class SAUpdateComponent implements OnInit {
  ...
  constructor(public router: Router, private route: ActivatedRoute, private saService: SAService) { }

  ngOnInit() {
    this.invoice = new Invoice();
    this.invoicePayment = new InvoicePaymentHistory();
    if (this.route.params['_value']['_id'] != "undefined") {
      this.route.params.switchMap((par: Params) => this.saService.fetchForPAIUpdate(par['_id'])).subscribe(x => {
        this.invoice = x;
  ...
  updateAsDraft() {
    ...
    this.saService.updateInvoice(this.invoice).subscribe(x => {
      this.router.navigateByUrl('transaction/pai/pai-list');
    })
  }

  save() {
    if (this.invoicePayment.paymentReceived > this.invoice.outstandingAmount) {
      ...
      this.invoicePayment.paymentMode = "Sundry Advances"
      this.invoicePayment.updatedBy =JSON.parse(localStorage.getItem('currentUser')).id
      this.saService.invoicePayment(this.invoicePayment).subscribe(x => {
  ...
  action() {
    this.router.navigateByUrl('transaction/sa/sa-list');
  }

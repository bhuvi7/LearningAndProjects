...
@Component({
    selector: 'fin-03a-inprogress',
    templateUrl: './fin-03a-inprogress.component.html',
    providers: [NgbModalConfig, NgbModal]
})

export class Fin03aInProgressComponent implements OnInit {
    ...
    selectedIds(selectedIds) {
        if (this.route.params['_value']['_indicator'] != "undefined") {
            if (this.route.params['_value']['_indicator'] == "new") {
                this.loading = true
                this.fin02aService.fetchDataForFin03aInProgress('APPROVED BY MOH', selectedIds).subscribe(fin => {
                ...
                this.fin02aService.fetchDataForFin03aInProgressOlder('APPROVED BY MOH', selectedIds).subscribe(fin => {
                    if (fin.length == 0) {
                        Swal.fire('', 'All the clinics under this state/district is Approved,check Approved process', 'info');
                        this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
                            dtInstance.destroy();
